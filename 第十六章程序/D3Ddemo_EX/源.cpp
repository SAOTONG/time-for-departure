//=================================================================================
// 描述：源.cpp文件，初步实现3D立方体的纹理映射，应用纹理过滤和纹理寻址。
// 并且可以使用鼠标键盘操作立方体。
//=================================================================================
...
#define WINDOW_TITLE	L"3D立方体纹理映射的实现以及纹理过滤、纹理寻址的应用"  // 为窗口标题定义的宏
...
LPDIRECT3DTEXTURE9      g_pTexture = NULL;       // 纹理缓存对象
...
HRESULT Objects_Init(HWND hwnd)
{
	...
	pVertex[1] = CUSTOMVERTEX(10.0f, 10.0f, -10.0f, 2.0f, 0.0f);
	pVertex[2] = CUSTOMVERTEX(10.0f, -10.0f, -10.0f, 2.0f, 2.0f);
	pVertex[3] = CUSTOMVERTEX(-10.0f, -10.0f, -10.0f, 0.0f, 2.0f);

	// 背面顶点数据
	pVertex[4] = CUSTOMVERTEX(10.0f, 10.0f, 10.0f, 0.0f, 0.0f);
	pVertex[5] = CUSTOMVERTEX(-10.0f, 10.0f, 10.0f, 2.0f, 0.0f);
	pVertex[6] = CUSTOMVERTEX(-10.0f, -10.0f, 10.0f, 2.0f, 2.0f);
	pVertex[7] = CUSTOMVERTEX(10.0f, -10.0f, 10.0f, 0.0f, 2.0f);

	// 顶面顶点数据
	pVertex[8] = CUSTOMVERTEX(-10.0f, 10.0f, 10.0f, 0.0f, 0.0f);
	pVertex[9] = CUSTOMVERTEX(10.0f, 10.0f, 10.0f, 2.0f, 0.0f);
	pVertex[10] = CUSTOMVERTEX(10.0f, 10.0f, -10.0f, 2.0f, 2.0f);
	pVertex[11] = CUSTOMVERTEX(-10.0f, 10.0f, -10.0f, 0.0f, 2.0f);

	// 底面顶点数据
	pVertex[12] = CUSTOMVERTEX(-10.0f, -10.0f, -10.0f, 0.0f, 0.0f);
	pVertex[13] = CUSTOMVERTEX(10.0f, -10.0f, -10.0f, 2.0f, 0.0f);
	pVertex[14] = CUSTOMVERTEX(10.0f, -10.0f, 10.0f, 2.0f, 2.0f);
	pVertex[15] = CUSTOMVERTEX(-10.0f, -10.0f, 10.0f, 0.0f, 2.0f);
	...
	g_pd3dDevice->SetLight(0, &light);   // 设置光照
	...
	// 各向异性过滤
	// 设置最大各向异性的程度值
	g_pd3dDevice->SetSamplerState(/*纹理层=*/0, D3DSAMP_MAXANISOTROPY, 3);
	// 设置放大过滤器，值为各向异性类型
...
VOID Direct3D_Update(HWND hwnd)
{
	...
	if (g_pDirectInput->IsKeyButtonDown(DIK_3))
	{
		// 设置夹取寻址模式
		g_pd3dDevice->SetSamplerState(0, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP);
		g_pd3dDevice->SetSamplerState(0, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP);
	}
	if (g_pDirectInput->IsKeyButtonDown(DIK_4))
	{
		// 设置边框颜色纹理寻址模式
