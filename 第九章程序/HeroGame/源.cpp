...
VOID HeroAction_Logic()
{
	...
	case ACTION_TYPE_NORMAL:
		if (1==rand()%4)
		{
			...
			swprintf_s(str, L"触发英雄一斩，对怪物造成【%d】点伤害值", damage);
		} 
		else
		{
			damage = 3 * (rand() % Hero.Agility) + Hero.Level*Hero.Strength + 20;
			Boss.NowHp -= (int)damage;
			swprintf_s(str, L"触发普通攻击，对怪物造成【%d】点伤害值", damage);
		}
		Message_Insert(str);
		break;
	case ACTION_TYPE_MAGIC:
		if (Hero.NowMp>=30)
		{
			...
			Boss.NowHp -= damage;
			swprintf_s(str, L"触发烈火剑法，对怪物造成【%d】点伤害值", damage);
		} 
		else
		{
			HeroActionType = ACTION_TYPE_MISS;
			swprintf_s(str, L"魔法值不足，无法发动该技能。", damage);
		}
		break;
	case ACTION_TYPE_RECOVER:
		if (Hero.NowMp>=40)
		{
			...
			swprintf_s(str, L"触发治疗术，玩家回复【%d】点生命值",HpRecover);
		} 
		else
		{
			HeroActionType = ACTION_TYPE_MISS;
...
VOID HeroAction_Paint()
{
	...
	case ACTION_TYPE_NORMAL:
		...
		TransparentBlt(g_mdc, 50, 170, 364, 140, g_bufdc, 0, 0, 364, 140, RGB(0, 0, 0));//透明色为RGB(0,0,0)
		break;
	case ACTION_TYPE_CRITICAL:
		SelectObject(g_bufdc, g_hHeroSkill3);
		TransparentBlt(g_mdc, 20, 60, 574, 306, g_bufdc, 0, 0, 574, 306, RGB(0, 0, 0));//透明色为RGB(0,0,0)
		break;
	case ACTION_TYPE_MAGIC:
		SelectObject(g_bufdc, g_hHeroSkill2);
		TransparentBlt(g_mdc, 50, 100, 374, 288, g_bufdc, 0, 0, 374, 288, RGB(0, 0, 0));//透明色为RGB(0,0,0)
		break;
	case ACTION_TYPE_MISS:
		break;
	case ACTION_TYPE_RECOVER:
		SelectObject(g_bufdc, g_hRecoverSkill);
		TransparentBlt(g_mdc, 560, 170, 150, 150, g_bufdc, 0, 0, 150, 150, RGB(0, 0, 0));//透明色为RGB(0,0,0)
		break;
	default:
		break;
	}
