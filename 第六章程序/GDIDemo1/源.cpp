...
HBITMAP g_hGirl[11];//声明位图数组，用来储存人物位图
...
BOOL Game_Init(HWND hWnd)
{
	...
	wchar_t filename[20];//用于存放文件名称的字符数组
	for (int i = 0; i < 11; i++)
	{
		memset(filename, 0, sizeof(filename));//初始化字符数组
		swprintf_s(filename, L"%d.bmp", i);//得到对应图像名称
		g_hGirl[i] = (HBITMAP)LoadImage(NULL, filename, IMAGE_BITMAP, 
			WINDOW_WIDTH, WINDOW_HEIGHT, LR_LOADFROMFILE);//加载该图像
	}
