...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	{
		MessageBox(hwnd, _T("DirectInput初始化失败~！"), _T("错误"), 0); //使用MessageBox函数，创建一个消息窗口 
		DestroyWindow(hwnd);
	}
...
VOID Matrix_Set()
{
	...
	D3DXMatrixPerspectiveFovLH(&matProj, D3DX_PI / 4.0f,
		(float)((double)WINDOW_WIDTH / WINDOW_HEIGHT), 1.0f, 10000.0f); //计算投影变换矩阵
