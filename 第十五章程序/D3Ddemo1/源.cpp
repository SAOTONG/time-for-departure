...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	g_pDirectInput->Init(hwnd, hInstance, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE,
...
VOID Direct3D_Update(HWND hwnd)
{
	...
	if (g_pDirectInput->IsKeyButtonDown(DIK_W))fPosY += 0.05f;
	if (g_pDirectInput->IsKeyButtonDown(DIK_A))fPosX -= 0.05f;
	if (g_pDirectInput->IsKeyButtonDown(DIK_S))fPosY -= 0.05f;
	if (g_pDirectInput->IsKeyButtonDown(DIK_D))fPosX += 0.05f;
	...
	if (g_pDirectInput->IsKeyButtonDown(DIK_UP))fAngleX += 0.05f;
	if (g_pDirectInput->IsKeyButtonDown(DIK_DOWN))fAngleX -= 0.05f;
	if (g_pDirectInput->IsKeyButtonDown(DIK_LEFT))fAngleY -= 0.05f;
	if (g_pDirectInput->IsKeyButtonDown(DIK_RIGHT))fAngleY += 0.05f;
...
VOID Matrix_Set()
{
	...
	D3DXVECTOR3 vEye(0.0f, 0.0f, -800.0f);  //摄像机的位置
	...
	D3DXMatrixPerspectiveFovLH(&matProj, D3DX_PI / 4.0f,
		(float)((double)WINDOW_WIDTH / WINDOW_HEIGHT), 1.0f, 1500.0f); //计算投影变换矩阵
